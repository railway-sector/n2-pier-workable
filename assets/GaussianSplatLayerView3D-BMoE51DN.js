...
return -0.5 * dot(conic, vec3(x * x, 2.0 * x * y, y * y));
  ...
  `)}const Zt=""+(se-1),Qt=`${Math.log2(se)}`,Ha=""+(ye-1),La=`${Math.log2(ye)}`;let mi=class extends Tt{constructor(){super(...arguments),this.tileCameraPosition=B(),this.cameraDelta=B()}};function gi(n){n.code.add(x`float unpackOpacity(uvec4 packedGaussian) {
